...
def subject_list(request):
    ...
    queryset_list = Subject.objects.active()  # .order_by("-timestamp")
...
def subject_detail(request, id=None):
...
def subject_update(request, id=None):
    # check this user has permission to do it - first_step
    if not request.user.is_authenticated:
        print("request.user.is_authenticated is False,", "AnonymousUser!")
        response = HttpResponse("You do not have permission to do this.")
...
def subject_delete(request, id=None):
    ...
    if request.method == "POST":
        parent_obj_url = reverse("subject:list")    # obj.get_absolute_url()
        instance.delete()
    ...
    return render(request, "subject_delete.html", context)
